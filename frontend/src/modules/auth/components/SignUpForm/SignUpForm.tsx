...
function SignUpForm() {
  ...
  const submitHandler = (e: any) => {
    e.preventDefault();
    if (!(emailValid(email) && pwdValid(pwd) && confirmPwdValid(confirmPwd))) { return; }
    ...
    signupRequest(email, pwd)
      .then((res) => {
        if (!res.data.apiError) {
          ...
          // все что возвращает api помимо "User already exist"
          // невозможно не user friendly и код ошибки одинаковый !!!
        ...
        <form className="sign-up-form__input-container" onSubmit={submitHandler}>
          <input
            ...
            onChange={(e) => {
              ...
              if (emailErr) { emailValid(e.target.value); }
              setRequestErr('');
            }}
            onBlur={(e) => {
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (pwdErr || confirmPwdErr) { pwdValid(e.target.value); }
              setRequestErr('');
            }}
            onBlur={(e) => {
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (confirmPwdErr) { confirmPwdValid(e.target.value); }
            }}
            onBlur={(e) => {
